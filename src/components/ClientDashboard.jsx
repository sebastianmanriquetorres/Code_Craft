...
import { Plus, Check, X, Users, Star, Info, Edit, Trash2, Mail, FolderOpen, Paperclip, Calendar } from 'lucide-react';
...
const ClientDashboard = ({ currentUser, users, projects, proposals, projectUpdates }) => {
  ...
  const currentProjects = useMemo(() => {
      ...
      return myProjects.slice(start, start + itemsPerPage);
  ...
  const updatesByProject = useMemo(() => {
    return projectUpdates.reduce((acc, update) => {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }}>
      ...
      <h2 className="text-2xl font-bold text-foreground mb-4">Mis Proyectos ({myProjects.length})</h2>
      
      <div className="grid md:grid-cols-2 gap-6 mb-6">
        {currentProjects.length === 0 ? <p className="text-muted-foreground col-span-2 text-center py-12">No tienes proyectos. ¡Crea uno!</p> : currentProjects.map(p => <ProjectCardClient key={p.id} project={p} proposals={projectProposals[p.id] || []} updates={updatesByProject[p.id] || []} users={users} onProposalResponse={handleProposalResponse} onLeaveReview={handleLeaveReview} onEdit={() => setEditingProject(p)} onDelete={() => setDeletingProject(p)} />)}
      </div>
...
const ProjectCardClient = ({ project, proposals, updates, users, onProposalResponse, onLeaveReview, onEdit, onDelete }) => {
  const [review, setReview] = useState({ rating: 5, comment: '' });
  const [showReview, setShowReview] = useState(false);
  const developer = users.find(u => u.id === project.developer_id);

  const pendingProposals = proposals.filter(p => p.status === 'pending');
  const sortedUpdates = useMemo(() => updates.sort((a, b) => new Date(b.created_at) - new Date(a.created_at)), [updates]);

  const getStatusInfo = () => {
    switch (project.status) {
      case 'open': return { className: 'bg-blue-100 text-blue-800 dark:bg-blue-900/50 dark:text-blue-300', text: 'Abierto' };
      case 'in-progress': return { className: 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900/50 dark:text-yellow-300', text: 'En progreso' };
      case 'completed': return { className: 'bg-green-100 text-green-800 dark:bg-green-900/50 dark:text-green-300', text: 'Finalizado' };
      default: return { className: 'bg-gray-100 text-gray-800', text: project.status };
    }
  };
  ...
  return (
    <motion.div layout className="bg-card border rounded-lg p-6 space-y-4 flex flex-col relative">
      <div className="flex justify-between items-start">
        <h3 className="font-bold text-lg">{project.title}</h3>
        <span className={`px-2 py-1 text-xs rounded-full font-semibold ${statusInfo.className}`}>{statusInfo.text}</span>
      </div>
      <p className="text-sm text-muted-foreground flex-grow">{project.description.substring(0, 120)}...</p>
      ...
      {project.status === 'open' && (
        <>
          ...
          {/* Updated Buttons Layout as per request */}
          <div className="grid grid-cols-2 gap-4 pt-4 border-t border-border/50">
            <Button variant="outline" size="sm" onClick={onEdit} className="w-full flex items-center justify-center">
                <Edit className="w-4 h-4 mr-2" />Editar
            </Button>
            <Button className="w-full bg-destructive hover:bg-destructive/90 text-white flex items-center justify-center" size="sm" onClick={onDelete}>
      ...
      {/* ... (Rest of component maintained for progress/completed states) */}
      {project.status === 'in-progress' && developer && (
        <div className="bg-secondary p-3 rounded-md">
            <p className="font-semibold">Dev: {developer.name} - ${project.price}</p>
            <div className="mt-2">
                <div className="flex justify-between text-sm font-medium mb-1"><span>Progreso</span><span>{project.progress || 0}%</span></div>
                <div className="w-full bg-border rounded-full h-2.5"><div className="bg-yellow-500 h-2.5 rounded-full" style={{ width: `${project.progress || 0}%` }}></div></div>
            </div>
        </div>
      )}
      {/* ... (Historial de Avances logic) ... */}
      {sortedUpdates.length > 0 && (
        <div className="space-y-3 pt-4 border-t">
            <h4 className="font-semibold text-md">Historial de Avances</h4>
            <div className="space-y-4 max-h-60 overflow-y-auto pr-2">
                {sortedUpdates.map(update => (
                    <div key={update.id} className="text-sm">
                        <div className="flex justify-between items-center mb-1">
                            <span className="font-semibold flex items-center gap-2"><Calendar className="w-4 h-4" /> {new Date(update.created_at).toLocaleDateString()}</span>
                            <span className="font-bold text-primary">{update.progress_percentage}%</span>
                        </div>
                        <p className="text-muted-foreground mb-2 pl-6">{update.description}</p>
                    </div>
                ))}
            </div>
        </div>
       ...
       {project.status === 'completed' && (
        <div className="flex items-center gap-2 text-sm bg-primary/10 text-primary p-3 rounded-md"><Info className="w-4 h-4 flex-shrink-0" /><span>¡Proyecto terminado!</span></div>
      )}
    </motion.div>
  );
};
