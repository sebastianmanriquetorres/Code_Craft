...
import { Plus, X, Upload, Link, Image as ImageIcon } from 'lucide-react';
...
const StorePage = ({ currentUser }) => {
  ...
  useEffect(() => {
    const fetchTemplates = async () => {
      const { data, error } = await supabase.from('dev_templates').select('*, profiles(name, avatar)');
      if (error) {
    ...
    const channel = supabase.channel('dev_templates_channel')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'dev_templates' }, (payload) => {
        if (payload.eventType === 'INSERT') {
          setTemplates(current => [...current, payload.new]);
        }
      })
      .subscribe();
  ...
  const handleAddToCart = (template) => {
    ...
    addItem(cartItem);
    toast({ title: "Plantilla añadida", description: `"${template.title}" está en tu carrito de proyectos.` });
  };
        ...
        {templates.map(template => (
          <motion.div key={template.id} className="bg-card border rounded-lg overflow-hidden group" whileHover={{ y: -5 }}>
            <div className="h-48 bg-muted overflow-hidden">
              <img class="w-full h-full object-cover transition-transform duration-300 group-hover:scale-105" alt={template.title} src="https://images.unsplash.com/photo-1702724122866-8898b15ebeac" />
            </div>
            <div className="p-6">
              ...
              <div className="flex items-center gap-2 mb-4">
                ...
                <span className="text-sm text-muted-foreground">{template.profiles?.name}</span>
              </div>
              ...
              <div className="flex gap-2">
                ...
                {currentUser.role === 'client' && <Button onClick={() => handleAddToCart(template)}>Usar esta plantilla</Button>}
              </div>
            </div>
          </motion.div>
        ))}
