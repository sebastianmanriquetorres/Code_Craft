...
const StorePage = ({ currentUser }) => {
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'dev_templates' }, (payload) => {
        if (payload.eventType === 'INSERT') {
          const fetchNewTemplate = async () => {
             ...
             if(!error && data){
                setTemplates(current => [data, ...current]); // Add new to top
             }
          }
          fetchNewTemplate();
  ...
  const handleCreateTemplate = async (e) => {
    ...
    if (error) {
      ...
      setShowForm(true); // Re-open if error
    } else {
  ...
  const handleAddToFavorites = (template) => {
    ...
    // Optimistic logic handled inside addToCart/useCart usually, 
    // but we trigger notification immediately
    toast({ title: "Añadiendo a favoritos..." });
        ...
        {paginatedTemplates.map(template => (
          <motion.div key={template.id} className="bg-card border rounded-lg overflow-hidden group" whileHover={{ y: -5 }}>
            <div className="h-48 bg-muted overflow-hidden">
              <img className="w-full h-full object-cover transition-transform duration-300 group-hover:scale-105" alt={template.title} src={template.image_url || "https://images.unsplash.com/photo-1702724122866-8898b15ebeac"} />
            </div>
            <div className="p-6">
